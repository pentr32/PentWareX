...
namespace PentWareX.Threaduri
{
    internal class Aimbot
    {
        public static void Run()
        {
            while (true)
            {
                ...
                for (int i = 0; i < maxPlayers; i++)
                {
                    ...
                    Settings.Aimbot.VisibleCheck = EntIsVisible(cEntity);
                }
        ...
        public static bool EntIsVisible(int ent)
        {
            int mask = MemoryManager.ReadMemory<int>(ent + Offsets.m_bSpottedByMask);
            int PBASE = MemoryManager.ReadMemory<int>(Structs.LocalPlayer.Base + 0x64) - 1;
